...
class DefaultAutoNATService {
    ...
    async handleIncomingAutonatStream(data) {
        ...
        try {
            ...
            const self = this;
            await pipe(source, (source) => lp.decode(source), async function* (stream) {
        ...
        try {
            ...
            const verifyAddress = async (peer) => {
                try {
                    ...
                    const buf = await pipe([request], (source) => lp.encode(source), source, (source) => lp.decode(source), async (stream) => first(stream));
                    if (buf == null) {
