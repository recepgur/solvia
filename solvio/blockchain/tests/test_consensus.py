...
async def test_validator_slashing():
    ...
    # Register validator with test mode enabled
    node_id = b"test_validator"
    stake = 5000
    node = ValidatorNode(node_id, stake)
    node._test_mode = True  # Enable test mode on validator
    ...
    # Enable test mode on consensus and start it
    consensus._test_mode = True
    await consensus.start()
    
    # Verify initial state
    assert node_id in consensus.active_set, "Validator should be in active set initially"
    assert not validator_info.is_slashed, "Validator should not be slashed initially"
    assert validator_info.stake == initial_stake, "Validator stake should be unchanged initially"
    
    # Simulate consecutive misses and force slashing
...
async def test_validator_rotation():
    ...
    for i in range(3):
        ...
        validators.append((node, stake))
        await consensus.register_validator(node, stake)
        # Ensure validator is in test mode after registration
        consensus.validators[node_id].node._test_mode = True
