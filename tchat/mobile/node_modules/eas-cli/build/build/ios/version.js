...
function validateShortVersion({ shortVersion, workflow, }) {
    if (shortVersion && !SHORT_VERSION_REGEX.test(shortVersion)) {
        if (workflow === eas_build_job_1.Workflow.MANAGED) {
            log_1.default.warn(`The required format for "version" field from app.json/app.config.ts for App Store iOS builds is one to three period-separated integers, such as 10.14.1. The string can only contain numeric characters (0-9) and periods. Current value: ${shortVersion}. Edit the "version" field in your app.json/app.config.ts to match the format required by Apple. ${(0, log_1.learnMore)('https://developer.apple.com/documentation/bundleresources/information_property_list/cfbundleshortversionstring')}`);
        }
        else {
            log_1.default.warn(`The required format for "CFBundleShortVersionString" in Info.plist for App Store iOS builds is one to three period-separated integers, such as 10.14.1. The string can only contain numeric characters (0-9) and periods. Current value: ${shortVersion}. Edit the "CFBundleShortVersionString" in your Info.plist to match the format required by Apple. ${(0, log_1.learnMore)('https://developer.apple.com/documentation/bundleresources/information_property_list/cfbundleshortversionstring')}`);
        }
    }
}
