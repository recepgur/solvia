...
def generate_exploit():
    ...
    if not vulnerability_info or "target" not in vulnerability_info:
        return (
            jsonify({"error": "No target or vulnerability information provided"}),
            400,
        )
    ...
    if "autonomousLearning" in vulnerability_info:
        advanced_options["autonomous_learning"] = vulnerability_info[
            "autonomousLearning"
        ]
    ...
    try:
        ...
        if exploit_code:
            ...
            return jsonify(
                {"exploit_code": exploit_code, "metrics": metrics, "status": "success"}
            )
        else:
            return (
                jsonify(
