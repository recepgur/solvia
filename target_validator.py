...
class TargetValidator:
    def __init__(self, whitelist_file: str = None):
        ...
        # Always convert to absolute path using the script's directory as base
        if not os.path.isabs(whitelist_file):
            base_dir = os.path.dirname(os.path.abspath(__file__))
            whitelist_file = os.path.join(base_dir, whitelist_file)

        self.whitelist_file = whitelist_file
        self.whitelist = self._load_whitelist()
        self.logger.info(
            f"Initialized TargetValidator with whitelist file: {self.whitelist_file}"
        )
    ...
    def is_target_allowed(self, target: str) -> bool:
        ...
        if target_domain and target_domain in whitelist["allowed_domains"]:
            self.logger.info(
                f"Target domain {target_domain} found in whitelist (domain match)"
            )
            return True
